...
![locate_anything_3peoplescene.jpg](images/locate_anything_3peoplescene.jpg)<br />*LocateAnything findings in a single image: original image; people, cars, streetlights, streetsigns, sneakers, coffee, drainpipe, curb, windows.*

---

## Installing `ComfyUI-LocateAnything` in RunComfy

*This is a step-by-step guide to getting the `ComfyUI-LocateAnything` node working on RunComfy.com. The node installs fine but has a runtime crash, as explained in Step 5 below, which requires a patch. The fix is to make two small edits to the model's Python source files. The 7 installation steps, described below, are:*

> 1. Install the `ComfyUI-LocateAnything` custom node
> 2. Install additional runtime dependencies 
> 3. Upload the `locate_anything_basic_demo.json` workflow
...
In ComfyUI Manager, search for **ComfyUI-LocateAnything** v.0.1.2 (by alisson-anjos) and install it. Restart ComfyUI after installation, and hard-refresh your browser. 

#### 2. Install additional runtime dependencies 

Open the RunComfy Terminal, and install the model's additional runtime dependencies:

`pip install decord lmdb`

#### 3. Upload the `locate_anything_basic_demo.json` workflow

![locate_anything_basic_demo_needs_metadata.png](workflows/locate_anything_basic_demo_needs_metadata.png)
...
* [`locate_anything_basic_workflowimg.png `](workflows/locate_anything_basic_demo_needs_metadata.png)

This will add the `LocateAnythingModelLoader` node to your workflow, and will set the following properties:
...
**Run** the workflow once. This will download the model weights (~6GB) to:

```
/workspace/ComfyUI/models/LocateAnything/nvidia--LocateAnything-3B/
```

It will also crash; *that's expected.* You will probably see an error like 
...
*These patches are stored as GitHib Gists. If you wish to modify or recreate the two Python patches, you will need to have a GitHub account.*
